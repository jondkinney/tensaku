...
/// Loupe limits. 1x is "no magnification", and past 4x a screenshot
/// region is mostly interpolation artefacts.
pub const MIN_SPOTLIGHT_MAGNIFICATION: f32 = 1.0;
pub const MAX_SPOTLIGHT_MAGNIFICATION: f32 = 4.0;
...
impl SpotlightKind {
    ...
    /// This shape's loupe factor, clamped to what the slider offers.
    fn magnification(&self) -> f32 {
        self.style().map_or(MIN_SPOTLIGHT_MAGNIFICATION, |s| {
            s.spotlight_magnification
                .clamp(MIN_SPOTLIGHT_MAGNIFICATION, MAX_SPOTLIGHT_MAGNIFICATION)
        })
    }
}
